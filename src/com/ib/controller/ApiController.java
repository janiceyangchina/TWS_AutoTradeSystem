...
public class ApiController implements EWrapper {
    ...
    public void reqDeepMktData(ArrayList<Contract> contracts, int numRows, IDeepMktDataHandler handler){
        if(!checkConnection())
            return;
        int reqId = m_reqId++;
        m_deepMktDataMap.put(reqId, handler);
        ...
        for(int i = 0; i < contracts.size(); i++){
            m_client.reqMktDepth(reqId, contracts.get(i), numRows, mktDepthOptions);
        }
    }

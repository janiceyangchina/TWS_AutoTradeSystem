...
class TopModel extends AbstractTableModel {
	private ArrayList<TopRow> m_rows = new ArrayList<TopRow>();
        ...
        void addRow( Contract contract) {
               String[] symbolList = contract.symbol().split(",");
                for(String symbol : symbolList){
                     symbol = symbol.trim();
                     Contract contractCombo1 = new Contract(contract.conid(), symbol, contract.getSecType(), contract.lastTradeDateOrContractMonth(),
                     contract.strike(), contract.getRight(), contract.multiplier(), contract.exchange(), contract.currency(), contract.localSymbol(), 
                     contract.tradingClass(), contract.comboLegs(), contract.primaryExch(), contract.includeExpired(), contract.getSecIdType(), contract.secId());
                     
                     TopRow row = new TopRow(this, contractCombo1.description());
                     m_rows.add(row);
                     
                     ApiDemo.INSTANCE.controller().reqTopMktData(contractCombo1, "", false, row);
                }
        ...
        void addRows(ArrayList<Contract> contracts){
            if(contracts == null || contracts.isEmpty())
                return;
            for(int i = 0; i < contracts.size() ; i++){
                
                TopRow row = new TopRow(this, contracts.get(i).description());
                m_rows.add(row);
